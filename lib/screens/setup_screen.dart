...
class _SetupScreenState extends State<SetupScreen> {
  ...
  Future<void> _loadPrevious() async {
    PrincipalConfig? previous;
    try { previous = await widget.store.loadConfig(); }
  ...
  @override
  void dispose() { host.dispose(); code.dispose(); super.dispose(); }

  Future<void> importBackup() async {
    var pasted = '';
    final raw = await showDialog<String>(context: context, builder: (context) => AlertDialog(
      title: const Text('Importer une sauvegarde locale'),
      content: TextField(onChanged: (value) => pasted = value, maxLines: 6, decoration: const InputDecoration(hintText: 'Collez le texte de la sauvegarde')),
      actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('Annuler')),
        FilledButton(onPressed: () => Navigator.pop(context, pasted), child: const Text('Importer'))],
    ));
    ...
    try {
      ...
      if (!mounted) return;
      if (config != null) { widget.onConnected(config); return; }
      setState(() => status = 'Saisies importées. Connectez-vous au professeur associé.');
  ...
  Future<void> connect({bool automatic = false}) async {
    ...
    if (enteredHost.isEmpty && !automatic) {
    ...
    try {
      final c = automatic
          ? await widget.api.discoverByCode(enteredCode, previous: await widget.store.loadConfig())
          : await widget.api.pairAddress(enteredHost, enteredCode);
                  ...
                  child: ConstrainedBox(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 14),
                        TextButton.icon(onPressed: busy ? null : () => connect(automatic: true), icon: const Icon(Icons.search), label: const Text('Rechercher le Principal avec mon code')),
                        TextButton.icon(onPressed: busy ? null : importBackup, icon: const Icon(Icons.file_download_outlined), label: const Text('Importer une sauvegarde')),
                        const Text(
