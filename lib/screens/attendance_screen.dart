...
class _AttendanceScreenState extends State<AttendanceScreen>
    with SafeSave<AttendanceScreen> {
  String kind = 'absence';
  ...
  Future<void> pickArrival() async {
    final value = await showTimePicker(
      context: context,
      initialTime: arrival ?? TimeOfDay.now(),
    );
  ...
  Future<void> _performSave() async {
    if (kind == 'retard' && arrival == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Indiquer l’heure d’arrivée pour un retard.'),
        ),
      );
      return;
    }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Saisie conservée localement jusqu’à synchronisation.'),
      ),
    );
        ...
        children: [
          ...
          DropdownButtonFormField<bool>(
            ...
            decoration: InputDecoration(
              labelText: kind == 'absence' ? 'Absence' : 'Retard',
            ),
            items: [
              DropdownMenuItem(
                value: true,
                child: Text(kind == 'absence' ? 'Justifiée' : 'Justifié'),
              ),
              DropdownMenuItem(
                value: false,
                child: Text(
                  kind == 'absence' ? 'Non justifiée' : 'Non justifié',
                ),
              ),
            ],
          ...
          TextField(
            ...
            decoration: const InputDecoration(
              labelText: 'Justification / remarque',
            ),
            maxLines: 3,
          ),
