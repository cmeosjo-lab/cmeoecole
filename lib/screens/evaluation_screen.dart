...
class _EvaluationScreenState extends State<EvaluationScreen>
    with SafeSave<EvaluationScreen> {
  ...
  List<Map<String, dynamic>> get periods {
    ...
    for (final raw in widget.snapshot.bulletinPeriods) {
      ...
      final allowed =
          ids is! List ||
          ids.isEmpty ||
          ids.map((e) => e.toString()).contains(widget.student.classId);
  ...
  String _periodLabel(Map<String, dynamic> p) {
    final name = (p['name'] ?? p['label'] ?? p['period'] ?? '')
        .toString()
        .trim();
  ...
  Future<void> _performSave() async {
    ...
    final invalid = scores.entries
        .where(
          (e) =>
              e.value != null &&
              (e.value! < 0 || e.value! > widget.snapshot.evaluationMax),
        )
        .toList();
    if (invalid.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Les notes doivent être comprises entre 0 et ${widget.snapshot.evaluationMax.toStringAsFixed(widget.snapshot.evaluationMax % 1 == 0 ? 0 : 1)}.',
          ),
        ),
      );
      return;
    }
    if (scores.values.every((v) => v == null) &&
        tajwidLevel == null &&
        appreciation.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Saisir au moins une note, un niveau de Tajwid ou une appréciation.',
          ),
        ),
      );
      return;
    ...
    final payload = <String, dynamic>{
      ...
      if (appreciation.text.trim().isNotEmpty)
        'appreciation': appreciation.text.trim(),
    };
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text(
          'Évaluation enregistrée. Synchroniser pour l’envoyer au Principal.',
        ),
      ),
    );
  ...
  Widget scoreField(
    ...
    decoration: InputDecoration(
      labelText:
          '$label / ${widget.snapshot.evaluationMax.toStringAsFixed(widget.snapshot.evaluationMax % 1 == 0 ? 0 : 1)}',
    ),
  );
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: SafeArea(
      child: ListView(
        ...
        children: [
          Text(
            'Évaluation — ${widget.student.displayName}',
            style: Theme.of(
              context,
          ...
          if (periods.isNotEmpty) ...[
            ...
            DropdownButtonFormField<String>(
              ...
              decoration: const InputDecoration(
                labelText: 'Période de bulletin (facultatif)',
              ),
              items: periods
                  .map(
                    (p) => DropdownMenuItem(
          ...
          DropdownButtonFormField<int>(
            initialValue: tajwidLevel,
            decoration: const InputDecoration(labelText: 'Niveau de Tajwid'),
            items: [1, 2, 3, 4, 5]
                .map(
                  (n) => DropdownMenuItem(value: n, child: Text('Niveau $n')),
                )
                .toList(),
