...
class _ClassLessonScreenState extends State<ClassLessonScreen> with SafeSave<ClassLessonScreen> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(appBar:AppBar(title:Text('Leçon — ${widget.schoolClass.name}')),body:SafeArea(child:ListView(padding:const EdgeInsets.all(16),children:[
      ...
      const SizedBox(height:12),DropdownButtonFormField<String>(initialValue:common,isExpanded:true,decoration:const InputDecoration(labelText:'État commun'),
        items:statuses.map((s)=>DropdownMenuItem(value:s,child:Text(s,overflow:TextOverflow.ellipsis))).toList(),onChanged:(v)=>setState(()=>common=v??'Fait')),
      const SizedBox(height:16),
      ...widget.students.map((s)=>Padding(padding:const EdgeInsets.only(bottom:16),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
        Text(s.displayName,style:const TextStyle(fontWeight:FontWeight.w600)),const SizedBox(height:8),
        DropdownButtonFormField<String>(key:ValueKey('${s.id}-$common'),initialValue:exceptions[s.id]??'',isExpanded:true,decoration:const InputDecoration(labelText:'État de cet élève'),
          items:[DropdownMenuItem(value:'',child:Text('Comme la classe : $common',overflow:TextOverflow.ellipsis)),...statuses.map((v)=>DropdownMenuItem(value:v,child:Text(v,overflow:TextOverflow.ellipsis)))],
