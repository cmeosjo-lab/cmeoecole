...
class SyncService {
  ...
  Future<SyncResult> synchronize(PrincipalConfig config) {
    final current = _inFlight[store]; if (current != null) return current;
    final future = _run(config);
    _inFlight[store] = future;
    return future;
  }
  Future<SyncResult> _run(PrincipalConfig initial) async {
    try { return await _synchronize(initial); }
    finally { _inFlight[store] = null; }
  }
  Future<SyncResult> _synchronize(PrincipalConfig initial) async {
    ...
    if (config.principalId.isNotEmpty && remoteId != config.principalId) {
      throw PrincipalApiException('Ce PC n’est pas le Principal enregistré. Aucune saisie n’a été envoyée.');
    }
    ...
    final sendable = queue.where((e) => e.supportedByPrincipal && e.teacher.trim().toLowerCase() == config.teacher.trim().toLowerCase()).toList();
    ...
    for (var start = 0; start < sendable.length; start += batchSize) {
      ...
      final ack = <String>{
        if (response['acknowledgedIds'] is List)
          ...(response['acknowledgedIds'] as List).map((v) => v.toString()).where(batchIds.contains),
      };
      ...
      if (response['rejectedItems'] is List) {
        for (final item in response['rejectedItems'] as List) {
          if (item is Map && batchIds.contains(item['id']?.toString())) {
            refused[item['id'].toString()] = (item['reason'] ?? 'Refus du Principal').toString();
          }
        }
      }
      await store.resolveQueue(acknowledgedIds: ack, rejectedReasons: refused, scope: config.scopeKey);
      sent += ack.length; received += _int(response['received']);
      duplicates += _int(response['duplicates']); rejected += refused.length;
      await store.appendSyncLog('Lot ${start + 1}–$end/${sendable.length} : ${ack.length} reçu(s), ${refused.length} refusé(s).');
      if (ack.isEmpty && refused.isEmpty) {
        throw PrincipalApiException('Le Principal n’a pas confirmé le lot. Les saisies sont conservées pour une nouvelle tentative.');
      }
    }
    ...
    final ids = history.where((e) => e.status == 'received').map((e) => e.id).toList();
    for (var start = 0; start < ids.length; start += 80) {
      ...
      try {
        ...
        final statuses = await api.eventStatuses(config, selected, deviceId: deviceId);
        statuses.removeWhere((key, _) => !selected.contains(key));
        await store.updateTransmissionStatuses(statuses, scope: config.scopeKey);
      } catch (_) {
        errors.add('Les décisions du Principal seront actualisées à la prochaine synchronisation.');
        break;
      }
    }
    final remaining = (await store.loadQueue(scope: config.scopeKey)).length;
    await store.markSuccessfulSync();
    return SyncResult(connected: true, sent: sent, received: received, rejected: rejected,
      duplicates: duplicates, unsupported: unsupported, remaining: remaining, errors: errors,
      snapshot: snapshot, message: remaining == 0 ? 'Toutes les saisies ont été transmises.' : '$remaining saisie(s) conservées à envoyer.');
  }
}
