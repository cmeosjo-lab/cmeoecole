...
class SyncCoordinator extends ChangeNotifier with WidgetsBindingObserver {
  ...
  void start() {
    ...
    _queueChanges = store.enqueued.listen((_) { _again = true; schedule(const Duration(seconds: 2)); });
    schedule(const Duration(seconds: 1));
  }
  void schedule(Duration delay) {
    _timer?.cancel();
    if (_active && !_disposed && !_maintenance) _timer = Timer(delay, synchronize);
  }
  Future<SyncResult?> synchronize() {
    if (_flight != null) return _flight!;
    if (_maintenance || _disposed) return Future.value(null);
    final f = _run(); _flight = f; return f;
  }
  Future<SyncResult?> _run() async {
    _timer?.cancel();
    busy = true; _again = false;
    if (!_disposed) notifyListeners();
    try {
      final config = await store.loadConfig();
      if (config == null) return null;
      final result = await SyncService(store, api).synchronize(config);
      lastResult = result; lastError = null; _failures = 0;
      return result;
    } catch (e) {
      lastError = e.toString(); _failures++;
      try { await store.appendSyncLog('Synchronisation différée : $e'); } catch (_) { /* Do not hide the original error. */ }
      return null;
    } finally {
      busy = false; _flight = null;
      if (!_disposed) {
        notifyListeners();
        final retrySeconds = _failures == 0 ? 90 : (5 * (1 << (_failures > 5 ? 5 : _failures - 1)));
  ...
  Future<void> maintain(Future<void> Function() action) async {
    ...
    _maintenance=true;_timer?.cancel();
    try { if(_flight!=null) await _flight; busy=true;if(!_disposed)notifyListeners();await action(); }
    finally {busy=false;_maintenance=false;if(!_disposed){notifyListeners();schedule(const Duration(seconds:1));}}
  }
  @override
  void didChangeAppLifecycleState(AppLifecycleState state) {
    _active = state == AppLifecycleState.resumed;
    if (_active) { schedule(const Duration(seconds: 1)); } else { _timer?.cancel(); }
  }
  @override
  void dispose() {
    _disposed = true; _timer?.cancel(); _queueChanges?.cancel();
