...
class _EcoleGestionProfAppState extends State<EcoleGestionProfApp> {
  ...
  @override
  void dispose() { coordinator.dispose(); store.close(); super.dispose(); }
  @override
  Widget build(BuildContext context) => MaterialApp(
    debugShowCheckedModeBanner: false,
    title: 'GESTCOURS Prof', theme: AppTheme.light(),
    home: !loaded ? const Scaffold(body: Center(child: CircularProgressIndicator()))
      : startupError != null ? Scaffold(appBar: AppBar(title: const Text('Protection des données')),
          body: Padding(padding: const EdgeInsets.all(24), child: SelectableText(
            'Les données locales n’ont pas pu être ouvertes. Aucune remise à zéro n’a été effectuée.\n\nNe désinstallez pas l’application.\n\n$startupError')))
      : config == null ? SetupScreen(store: store, api: api, onConnected: (c) {
          if (!mounted) return;
          setState(() => config = c); coordinator.schedule(const Duration(seconds: 1));
        })
      : HomeScreen(config: config!, store: store, api: api, coordinator: coordinator,
          onDisconnect: () { if (mounted) setState(() => config = null); }),
  );
}
